...
def validate_build_version(project_path):
    """Compare package.json version with build/appinfo.json when present."""
    package_path = project_path / 'package.json'
    appinfo_path = project_path / 'build' / 'appinfo.json'

    if not appinfo_path.exists():
        print_status('info', 'build/appinfo.json not found (build version not checked)')
        return []
    ...
    try:
        ...
        warning = f'Could not read package.json for version check: {e}'
        print_status('warning', warning)
        return [warning]
    ...
    try:
        ...
        warning = f'Could not read build/appinfo.json for version check: {e}'
        print_status('warning', warning)
        return [warning]

    package_version = pkg.get('version')
    build_version = appinfo.get('versionLabel')

    if not package_version:
        warning = 'package.json has no version field'
        print_status('warning', warning)
        warnings.append(warning)
    elif not build_version:
        warning = 'build/appinfo.json has no versionLabel field'
        print_status('warning', warning)
        warnings.append(warning)
    elif package_version == build_version:
        print_status('ok', f'Build version matches package.json ({package_version})')
    else:
        warning = (
            f'build/appinfo.json versionLabel is {build_version}, '
            f'but package.json version is {package_version}; rebuild after bumping version'
        )
        print_status('warning', warning)
        warnings.append(warning)
...
def main():
    ...
    print(f"\n{Colors.BOLD}Checking build metadata...{Colors.RESET}")
    validate_build_version(project_path)
